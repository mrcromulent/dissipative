 ...
 *
 * \param field Specifies the field as an enumeration type iofields (allvars.h), e.g. IO_POS. Don't forget to insert new fields also in allvars.h.
 * \param label The label of the dataset (4 characters)
 * \param datasetname The name of the hdf5 dataset (maximum 256 characters)
 * \param type_in_memory The type of the field in the memory (use MEM_NONE if specifying io_func)
 * \param type_in_file_output The output type in the hdf5 file
 * \param type_in_file_input The input type in the hdf5 file (use FILE_MY_OUTPUT_TYPE for MyInputType, input is disabled with FILE_NONE)
 * \param values_per_block The number of values per field, e.g. 1 for mass, 3 for velocities
 * \param array The array in which the value is stored. For an io_func this influences the particle index,
                the default (A_NONE) is an index into P/SphP, can be changed to indexes into StarP/BHP/TLL using A_STARP/A_BHP/A_TLL
 * \param pointer_to_field A Pointer to the field in one of the global arrays, e.g. &SphP[0].Density, or &P[0].Vel[0]
 * \param io_func Alternatively, if the value to output/input is not a simple field, you can define a function which handles i/o
 * \param typelist_bitmask Specifies for which particle type the field is present, e.g. 1+2+8 => field present for particle types 0,1,3 (or use ALL_TYPES, GAS_ONLY,...)
 */
void init_field(enum iofields field, const char *label, const char *datasetname,
                ...
                enum types_in_file type_in_file_input, int values_per_block, enum arrays array,
                void *pointer_to_field, void (*io_func) (int, int, void *, int),
 ...
 *
 * \param field Specifies the field as an enumeration type iofields (allvars.h), e.g. IO_POS.
 * \param a the exponent of the cosmological a factor
 * \param h the exponent of the hubble parameter
 * \param L the length unit scaling
 * \param M the mass unit scaling
 * \param V the velocity unit scaling
 * \param c conversion factor to cgs units (zero indicates dimensionless quantity, integer count, etc)
 */
...
/*!
 * \brief Function for determining whether a field is dumped in mini snapshots
 * This only works for fields registered with init_field
 * The member snap_type is initialized to SN_FULL in init_field
 *
 * \param field Specifies the field as an enumeration type iofields (allvars.h), e.g. IO_POS.
 * \param type In which snapshot types this field should be present (e.g. SN_FULL, SN_MINI, SN_NO_SUBBOX)
*/
 ...
 *
 *  If a write error occurs, this function prints some useful
 *  debug information and sets to 1 the variable WriteErrorFlag
 *  so that the write operation that caused the error can be
 *  performed again
 *
 *  \param check flag that indicates where the function was called [0 and 1 in my_fwrite(), 2 in my_hdf5_error_handler(), 3 in hdf5_header_error_handler()]
 *  \param nwritten number of elements actually written
 *  \param nmemb number of elements that should be written
 */
  ...
  if(DumpFlag)
    {
...
#ifdef FOF
      if(RestartFlag != 3 && RestartFlag != 15 && RestartFlag != 18 && subbox_flag == 0 && DumpFlag != 2)
        {
#ifdef TGSET
          if(TGD.NHMax < 1e4)
#endif
      ...
      for(n = 0; n < NumPart; n++)
        {
          ...
          if(subbox_flag > 0)
            if(P[n].Pos[0] < SubboxXmin[subbox_flag - 1] || P[n].Pos[0] > SubboxXmax[subbox_flag - 1] ||
               P[n].Pos[1] < SubboxYmin[subbox_flag - 1] || P[n].Pos[1] > SubboxYmax[subbox_flag - 1] || P[n].Pos[2] < SubboxZmin[subbox_flag - 1] || P[n].Pos[2] > SubboxZmax[subbox_flag - 1])
              continue;
#endif
          n_type[P[n].Type]++;
        }
          ...
          while(next >= 0)
            {
              ...
              if(subbox_flag > 0)
                if(P[i].Pos[0] < SubboxXmin[subbox_flag - 1] || P[i].Pos[0] > SubboxXmax[subbox_flag - 1] ||
                   P[i].Pos[1] < SubboxYmin[subbox_flag - 1] || P[i].Pos[1] > SubboxYmax[subbox_flag - 1] || P[i].Pos[2] < SubboxZmin[subbox_flag - 1] || P[i].Pos[2] > SubboxZmax[subbox_flag - 1])
      ...
      for(n = 0; n < NumPart; n++)
        {
          if(P[n].Type == 4 && STP(n).BirthTime <= 0)
            {
              N_wind++;
          ...
          if(ThisTask == 0)
            {
              ...
              if(subbox_flag > 0)
		{
		  sprintf(buf, "%s/subbox%d", All.OutputDir, subbox_flag - 1);
		  mkdir(buf, 02755);
		  sprintf(buf, "%s/subbox%d/snapdir_subbox%d_%03d", All.OutputDir, subbox_flag - 1, subbox_flag - 1, num);
		}
#endif
              mkdir(buf, 02755);
              ...
              if(subbox_flag > 0)
		{
		  sprintf(alternative_fname, "%s/subbox%d", AlternativeOutputDir, subbox_flag - 1);
		  mkdir(alternative_fname, 02755);
		  sprintf(alternative_fname, "%s/subbox%d/snapdir_subbox%d_%03d", AlternativeOutputDir, subbox_flag - 1, subbox_flag - 1, num);
		}
#endif
              mkdir(alternative_fname, 02755);
#endif
      ...
      if(subbox_flag > 0)
        {
          if(All.NumFilesPerSnapshot > 1)
            sprintf(buf, "%s/subbox%d/snapdir_subbox%d_%03d/%s_subbox%d_%03d.%d", All.OutputDir, subbox_flag - 1, subbox_flag - 1, num, All.SnapshotFileBase, subbox_flag - 1, num, filenr);
          else
            sprintf(buf, "%s%s_subbox%d_%03d", All.OutputDir, All.SnapshotFileBase, subbox_flag - 1, num);
        }
#ifdef TOLERATE_WRITE_ERROR
      if(subbox_flag > 0)
        {
          if(All.NumFilesPerSnapshot > 1)
            sprintf(alternative_fname, "%s/subbox%d/snapdir_subbox%d_%03d/%s_subbox%d_%03d.%d", AlternativeOutputDir, subbox_flag - 1, subbox_flag - 1, num, All.SnapshotFileBase, subbox_flag - 1, num, filenr);
          else
            sprintf(alternative_fname, "%s%s_subbox%d_%03d", AlternativeOutputDir, All.SnapshotFileBase, subbox_flag - 1, num);
        }
#endif
#endif
      ...
      if(RestartFlag == 11)
        {
          ...
          if(All.NumFilesPerSnapshot > 1)
            sprintf(buf, "%s/snapdir_%03d/%s-TRpart_%03d.%d", All.OutputDir, num, All.SnapshotFileBase, num, filenr);
          else
            sprintf(buf, "%s%s-TRpart_%03d", All.OutputDir, All.SnapshotFileBase, num);
#endif
        }
...
#ifdef FOF
      if(RestartFlag != 3 && RestartFlag != 15 && RestartFlag != 6 && RestartFlag != 18 && subbox_flag == 0 && DumpFlag != 2)
        {
#ifdef TGSET
          if(TGD.NHMax < 1e4)
#endif
...
#if defined(VORONOI_MESHOUTPUT) /* after mesh is recreated */
      for(gr = 0; gr < ngroups; gr++)
        {
          if(All.NumFilesPerSnapshot > 1)
...
#ifdef TRACER_PARTICLE
              if(All.NumFilesPerSnapshot > 1)
                sprintf(buf, "%s/snapdir_%03d/voronoi_mesh-TRpart_%03d.%d", All.OutputDir, num, num, filenr);
              else
                sprintf(buf, "%s/voronoi_mesh-TRpart_%03d", All.OutputDir, num);
#endif
            }
 ...
 *
 *  \param blocknr ID of the output block (i.e. position, velocities...)
 *  \param mode  (for input mode > 0, for output mode = 0)
...
int get_particles_in_block(enum iofields blocknr, int *typelist)
{
  ...
  switch (blocknr)
    ...
    case IO_AGE:
      for(i = 0; i < NTYPES; i++)
        if((All.ICFormat == 3 && (i == 0 || i == 1 || i == 5)) || (All.ICFormat != 3 && i != 4))
          typelist[i] = 0;
      return header.npart[4] + (All.ICFormat == 3 ? (header.npart[2] + header.npart[3]) : 0);
      break;
    ...
    case IO_Z:
      for(i = 0; i < NTYPES; i++)
        if((All.ICFormat == 3 && (i == 1 || i == 5)) || (All.ICFormat != 3 && (i != 0 && i != 4)))
          typelist[i] = 0;
      return header.npart[0]+header.npart[4] + (All.ICFormat == 3 ? (header.npart[2] + header.npart[3]) : 0);
      break;
...
/*! \brief Check if a block is expected for file input or output.
 *
 ...
 *
 *  \param blocknr ID of the output block (i.e. position, velocities...)
 *  \param write if 0 the function is in read mode, if 1 the function is in write mode
...
int blockpresent(enum iofields blocknr, int write)
{
  ...
  if(!write)
    {
#ifdef TGSET
      if((RestartFlag == 0 || RestartFlag == 3) && blocknr > IO_MASS)
#else
...
#else
#ifdef CR_IC
      if(RestartFlag == 0 && ((blocknr > IO_CR_Q0 && blocknr != IO_BFLD) || (blocknr >= IO_RHO && blocknr <= IO_ACCEL)))
#else
#ifndef CHEMISTRY
...
#ifdef BECDM
  hdf5_dataspace = my_H5Screate(H5S_SCALAR);
  hdf5_attribute = my_H5Acreate(handle, "AxionMassEv", H5T_NATIVE_DOUBLE, hdf5_dataspace, H5P_DEFAULT);
  my_H5Awrite(hdf5_attribute, H5T_NATIVE_DOUBLE, &All.AxionMassEv, "AxionMassEv");
  my_H5Aclose(hdf5_attribute, "AxionMassEv");
  my_H5Sclose(hdf5_dataspace, H5S_SCALAR);
#endif
 ...
 *
 * \param fname name of the snapshot
 */
...
void open_image_files(char *id1, int num, FILE ** dens, FILE ** temp, FILE ** met, FILE ** vel, FILE ** mag, FILE ** vort, FILE ** phot, FILE ** chem,
                      FILE ** denstr, FILE ** dust, FILE ** h2, FILE ** hp, FILE ** co)
{
  if(ThisTask != 0)
    return;
  ...
  if(dens)
    {
      sprintf(buf, "%s/density_%s_%03d", All.OutputDir, id1, num);
      *dens = open_file(buf);
    }

  if(temp)
    {
      sprintf(buf, "%s/temp_%s_%03d", All.OutputDir, id1, num);
      *temp = open_file(buf);
    }

  if(met)
    {
      sprintf(buf, "%s/metal_%s_%03d", All.OutputDir, id1, num);
      *met = open_file(buf);
    }

  if(vel)
    {
      sprintf(buf, "%s/velocity_%s_%03d", All.OutputDir, id1, num);
      *vel = open_file(buf);
    }

  if(mag)
    {
      sprintf(buf, "%s/magnetic_%s_%03d", All.OutputDir, id1, num);
      *mag = open_file(buf);
    }

  if(vort)
    {
      sprintf(buf, "%s/vorticity_%s_%03d", All.OutputDir, id1, num);
      *vort = open_file(buf);
    }

  if(phot)
    {
      sprintf(buf, "%s/photondens_%s_%03d", All.OutputDir, id1, num);
      *phot = open_file(buf);
    }

  if(chem)
    {
      sprintf(buf, "%s/chemelements_%s_%03d", All.OutputDir, id1, num);
      *chem = open_file(buf);
    }

  if(denstr)
    {
      sprintf(buf, "%s/density_trmc_%s_%03d", All.OutputDir, id1, num);
      *denstr = open_file(buf);
    }

  if(dust)
    {
      sprintf(buf, "%s/tdust_%s_%03d", All.OutputDir, id1, num);
      *dust = open_file(buf);
    }

  if(h2)
    {
      sprintf(buf, "%s/xH2_%s_%03d", All.OutputDir, id1, num);
      *h2 = open_file(buf);
    }

  if(hp)
    {
      sprintf(buf, "%s/xHP_%s_%03d", All.OutputDir, id1, num);
      *hp = open_file(buf);
    }

  if(co)
    {
      sprintf(buf, "%s/xCO_%s_%03d", All.OutputDir, id1, num);
      *co = open_file(buf);
    }
...
void read_subbox_coordinates(char *fname)
{
  ...
  for(iter = 0, i = 0; iter < 2; iter++)
    {
      if(!(fsubboxes = fopen(fname, "r")))
        terminate("SUBBOX_SNAPSHOTS: cannot read subboxes coordinates in file `%s'\n", fname);
      if(iter == 0)
        while(fscanf(fsubboxes, "%lg %lg %lg %lg %lg %lg", &dummy, &dummy, &dummy, &dummy, &dummy, &dummy) != EOF)
          Nsubboxes++;
      if(iter == 1)
        while(fscanf(fsubboxes, "%lg %lg %lg %lg %lg %lg", &(SubboxXmin[i]), &(SubboxXmax[i]), &(SubboxYmin[i]), &(SubboxYmax[i]), &(SubboxZmin[i]), &(SubboxZmax[i])) != EOF)
          i++;
      fclose(fsubboxes);
  ...
  for(i = 0; i < Nsubboxes; i++)
    mpi_printf("SUBBOX_SNAPSHOTS: subbox #%d coordinates: %g %g %g %g %g %g\n", i, SubboxXmin[i], SubboxXmax[i], SubboxYmin[i], SubboxYmax[i], SubboxZmin[i], SubboxZmax[i]);
  mpi_printf("\n");
}
#endif
